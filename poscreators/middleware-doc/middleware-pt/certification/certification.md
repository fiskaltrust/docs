...
On this route fiskaltrust is the producer of the certified program. Based on the existing implementation of the fiskaltrust.CloudCashBox, the following components are provided and operated by fiskaltrust; PosCreators do not have to build, configure, or defend them towards the AT:

| Component | What fiskaltrust does |
| --------- | --------------------- |
...
| **Series and ATCUD** | Document series are created for every queue and registered with the AT through the series webservice; the ATCUD validation codes are obtained and applied automatically. |
...
| **Product master data** | The product table of the SAF-T (PT) is derived from the charge items (unique product code per article, trimmed and validated descriptions). No separate product master has to be maintained in the Middleware. |
| **Tax table and exemption reasons** | The SAF-T tax table with the supported VAT rates and the exemption codes is generated by the Middleware; only valid exemption reasons are accepted. |
...
| **Fiscal data** | Numbering, hash chain, hash extract, certificate line, QR code content, and all mandatory texts are produced by the Middleware. |
| **SAF-T (PT)** | The complete audit file (header with company data and certificate number, master files, source documents, working documents, payments) is produced on request through the journal endpoint. |
| **Document rendering** | The certified layout is rendered by fiskaltrust; the print format is protected against changes by the merchant or the POS. |
| **Operations** | Hosting in the fiskaltrust cloud, backups, system time, availability, and the adaptation of the program when the regulation changes. |
