...
- Click on the site in which your device is enrolled
  ![Orderman SystemCenterNext Sites list with an arrow pointing to the site to select](images/01_select_site.png)<br/>*Figure 1. Selecting the site in which the device is enrolled in the Orderman SCN Portal.*
- Click on your device to get to the device details
  ![Orderman SystemCenterNext site Devices list with an arrow pointing to the device OM1](images/02_select_device.png)<br/>*Figure 2. Opening the device details in the Orderman SCN Portal.*
- Click on the `Config` button on the right
  ![Orderman SystemCenterNext device page with an arrow pointing to the Config button under Actions](images/03_click_config.png)<br/>*Figure 3. Opening the device Config in the Orderman SCN Portal.*

### Step 3: Choose the app to install

- Navigate to the `Applications` card and click on `+ INSTALL` button on the right
  ![Orderman device Config Applications card with an arrow pointing to the INSTALL button](images/04_click_install.png)<br/>*Figure 4. Starting an app install from the Applications card.*
- Navigate to the `Public files`
  ![Orderman Install Application dialog with an arrow pointing to the Public files tab](images/05_click_public_files.png)<br/>*Figure 5. Navigating to the Public files section.*
- Scroll and find the `fiskaltrust InStore App` entry. To install the latest available version just click this entry.
  ![Orderman Install Application public files list with an arrow pointing to the fiskaltrust InStore App entry](images/06_select_app.png)<br/>*Figure 6. Selecting the fiskaltrust InStore App entry to install the latest version.*
...
![Orderman Applications list entry for the fiskaltrust InStore App showing version none to 1.2.6](images/07_app_list_entry.png)
...
![Orderman device Config page with an arrow pointing to the APPLY button at the bottom left](images/08_apply.png)
