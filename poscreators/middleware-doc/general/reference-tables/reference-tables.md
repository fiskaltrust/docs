...
| Value | Description |
|-------|-----------------|
| `0010` | **Request additional information (hardware / device details) from the SCU**<br />Returns information about the SCU/security device in the response's `ftStateData` field. The returned object is market-specific: `TseInfo` in Germany, `RTInfo` in Italy, and `SignatureCertificate` in Austria and France. |
