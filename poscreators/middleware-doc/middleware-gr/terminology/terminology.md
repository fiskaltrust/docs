...
| myDATA | *my Digital Accounting and Tax Application.* AADE's platform that maintains the electronic books of Greek businesses. Businesses transmit invoice and receipt data to myDATA, which validates the submission, records it, and returns a unique registration number known as the MARK. Greece also knows certified fiscal devices (ΦΗΜ); on the provider route implemented by the fiskaltrust.Middleware, however, no local fiscal device is involved — fiscalization is an online API call to the tax authority through a licensed provider. |
