...
The **fiskaltrust POS System API** is the HTTP/JSON interface between a POS system and the **fiskaltrust.Middleware**. All country-specific fiscal logic (signing, hash chaining, audit-trail formatting) is handled by the Middleware behind the same set of endpoints — POS systems integrate once and run across all supported markets.
...
The POS System API is always accessed in the context of a CashBox. Each request must include four headers covering authentication, identification, and idempotency:
...
The first three are issued via the **fiskaltrust Portal** as part of the CashBox configuration and must be stored securely on the POS side. The `x-operation-id` is generated by the POS system per logical operation — typically a fresh UUID — and reused unchanged on retries.
...
In this setup, the POS system communicates with the **Cloud PosSystem API**, backed by a Cloud CashBox. All five endpoint groups are reachable: `/echo`, `/sign`, and `/journal` route to the Middleware directly (and are also available in the Local Helper deployments in Options 2 and 3), while `/pay` and `/issue` are handled by the Cloud CashBox.
